...
export default function Login() {
  ...
  // Initialize React Hook Form with Zod schema and real-time onChange mode
  const { register, handleSubmit, setValue, trigger, formState: { errors } } = useForm({
    resolver: zodResolver(loginSchema),
    mode: 'onChange',
    defaultValues: {
      email: ROLES[0].credentials.hint,
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(updateTabIndicator, 120);
    return () => {
  ...
  // Handle switching between role selection tabs
  const handleRoleChange = async (idx) => {
    setSelectedRoleIdx(idx);
    setApiError('');
    setValue('email', ROLES[idx].credentials.hint);
    setValue('password', 'password123');
    // Clear validation messages on switch
    await trigger();
  };

  // Submit Handler connected to backend via AuthContext axios call
  const onSubmit = async (data) => {
    setApiError('');
    setIsLoading(true);

    try {
      const userData = await setAuthSession(data.email, data.password);
      setIsLoading(false);
      setToastMsg(`Access Granted: Redirecting to ${userData.role} portal...`);
      setToastVisible(true);

      setTimeout(() => {
        setToastVisible(false);
        // Dynamically redirect user based on their role using react-router-dom
        if (userData.role === 'Service Technician') {
          navigate('/return');
        } else if (userData.role === 'Accounts Staff') {
          navigate('/settlement');
        } else {
          navigate('/dashboard');
        }
      ...
      setApiError(responseError || 'Invalid email address or password.');
    }
  };
    ...
    <div style={{
      ...
      {isLargeScreen && (
        <div style={{
          ...
          <Orb style={{ top: '-120px', left: '-100px', width: '420px', height: '420px', background: 'rgba(99,102,241,0.22)' }} />
          <Orb style={{ bottom: '-80px', right: '-80px', width: '350px', height: '350px', background: 'rgba(59,130,246,0.16)' }} />
          <Orb style={{ top: '40%', left: '60%', width: '200px', height: '200px', background: 'rgba(139,92,246,0.14)' }} />
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ padding: isLargeScreen ? '24px 32px 32px' : '18px 20px 20px' }}>
              ...
              {/* Form Input fields managed by react-hook-form */}
              <form onSubmit={handleSubmit(onSubmit)} noValidate>
                ...
                {/* ── API Response Errors (e.g. 401 Unauthorized) ── */}
                {apiError && (
                  <div style={{
                    color: '#ef4444',
                    fontSize: '0.78rem',
